...
#
# def euclidean_distances(x, y, squared=False):#计算两个点之间的欧氏距离
#     """
...
#             if res.resname == 'GOA':
#                 # 如果是GOA，输出残基序号
#                 print(f"Residue GOA found at position {res.get_id()[1]}")  # 获取残基位置
#             # 只添加标准的氨基酸残基
#             if res.resname in protein_letters_3to1:
#                 return protein_letters_3to1[res.resname]
#             else:
#                 # 如果是非标准残基（例如GOA），可以跳过或使用 'X'
#                 return 'X'  # 这里 'X' 代表无法识别的残基
#
#
...
class StructureDataParser:
    ...
    def generate_atom_distance_map(self, atom='CA'):
        coords_ = self.generate_residue_coordinate(atom)
        #coords_, _ = self.generate_residue_coordinate(atom)
        return euclidean_distances(coords_, coords_)
    ...
    #         if atom in res and res.resname in protein_letters_3to1:
    #             coord_list.append(res[atom].coord)
    #             index_list.append(i)  # 原始结构序列中的第 i 个残基
    #     coords_ = np.stack(coord_list)
    #     return coords_, index_list
    # def generate_residue_coordinate(self, atoms=['CA','N','C']):
    #     coord_list = []
    #
    #     for res in self.structure.get_residues():
    ...
    #
    #     for atom in atoms:
    #         coords_array, indices = self.generate_residue_coordinate(atom)
    #         coords[atom] = coords_array.tolist()
    #         index_map[atom] = indices  # 记录该原子的残基索引
    #
...
#         coord_list = []
#         for res in self.structure.get_residues():
#             if atom in res:  # 检查该残基是否有所需的原子
#                 coord_list.append(res[atom].coord)
#             else:
#                 print(f"Warning: Residue {res.get_resname()} {res.get_id()[1]} does not have {atom} atom.")
#         coords_ = np.stack(coord_list)
#         return coords_
#
#     # def generate_residue_coordinate(self, atoms=['CA','N','C']):
